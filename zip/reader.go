...
type File struct {
	...
	zip           *Reader
	zipr          io.ReaderAt
	headerOfHfset int64 // includes overall ZIP archive baseOffset
	HeaderOffset  int64
...
func (f *File) DataOffset() (offset int64, err error) {
	...
	return f.headerOfHfset + bodyOffset, nil
}
...
func (f *File) Open() (io.ReadCloser, error) {
	...
	r := io.NewSectionReader(f.zipr, f.headerOfHfset+bodyOffset, size)
	...
	if f.hasDataDescriptor() {
		desr = io.NewSectionReader(f.zipr, f.headerOfHfset+bodyOffset+size, dataDescriptorLen)
	}
	rc = &checksumReader{
		rc:   rc,
		hash: crc32.NewIEEE(),
...
func (f *File) OpenRaw() (io.Reader, error) {
	...
	r := io.NewSectionReader(f.zipr, f.headerOfHfset+bodyOffset, int64(f.CompressedSize64))
	return r, nil
}
...
func readDirectoryHeader(f *File, r io.Reader) error {
	...
	needHeaderOffset := f.headerOfHfset == int64(^uint32(0))
	...
	for extra := readBuf(f.Extra); len(extra) >= 4; { // need at least tag and size
		...
		case zip64ExtraID:
			...
			if needHeaderOffset {
				...
				f.headerOfHfset = int64(fieldBuf.uint64())
			}
		case ntfsExtraID:
			if len(fieldBuf) < 4 {
